...
fileTemp = [tempfolder 'tempVid6.mp4'];
if ispc == 1;
...
Vid6 = VideoWriter(fileTemp, profile);
Vid6.Quality = 85; %cannot take higher
Vid6.FrameRate = targetRate;
open(Vid6);
...
%                 end;
            ...
            if tempRate < targetRate;
                ...
                frame1 = frame;
                frame2 = frame;
                writeVideo(Vid6, frame1);
                writeVideo(Vid6, frame2);
            else;
                writeVideo(Vid6, frame);
            end;

            if mod(iterVid6,10) == 0;
                %Save image count every 10 iterations
                if ismac == 1;
                    save('/Applications/SP2VideoManager/Temp/outputVid6.mat', 'finishVid6', 'iterVid6');
                elseif ispc == 1;
                    save([MDIR '\SP2VideoManager\Temp\outputVid6.mat'], 'finishVid6', 'iterVid6');
                end;
            end;
            iterVid6 = iterVid6 + 1;
        end;
