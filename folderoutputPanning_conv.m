function [] = folderoutputPanning_conv(varargin);
...
path = uigetdir('Panning videos output folder', handles.defaultpathoutputPanning);
...
handles.defaultpathoutputPanning = path;

disppath = path;
set(handles.pathoutputPanning_conv, 'String', disppath, 'tooltip', path);
posExtent = get(handles.pathoutputPanning_conv, 'Extent');
posReal = get(handles.pathoutputPanning_conv, 'Position');
...
set(handles.pathoutputPanning_conv, 'String', disppath);
drawnow;
...
handles.outputfolderPanning = path;
...
if ischar(handles.inputfolderPanning) == 1 & ischar(handles.outputfolderPanning) == 1;
    ...
    for i = 1:length(listDir);
        ...
        if isempty(lidot) == 1;
            ...
            fileECext = fileEC.name(lidot:end);
        end;
...
if pathfold ~= 0;
    handles.OutputPanningLocation = 'Disk';
