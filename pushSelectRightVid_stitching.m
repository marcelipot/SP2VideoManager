function [] = pushSelectRightVid_stitching(varargin);
...
disppath = fileEC;
set(handles.filenameRightvid_stitching, 'String', disppath);
posExtent = get(handles.filenameRightvid_stitching, 'Extent');
posReal = get(handles.filenameRightvid_stitching, 'Position');
if posExtent(3) >= posReal(3);
    disppath = [disppath(1:10) ' ... ' disppath(end-10:end)];
    set(handles.filenameRightvid_stitching, 'String', disppath);
end;
if ispc == 1;
    handles.pathRightfile_stitching = [path '\' fileEC];
    set(handles.filenameRightvid_stitching, 'tooltipstring', handles.pathRightfile_stitching);
elseif ismac == 1;
...
if ismac == 1;
    ...
    command = ['"' ffmpegfolder '" -i "' handles.pathRightfile_stitching ...
...
if status == 1;
    ...
    set(handles.filenameRightvid_stitching, 'tooltip', '', 'String', '');
end;
...
%---Get video
fileEC = handles.pathRightfile_stitching;
VidInfo.name = fileEC;
...
xlim(handles.mainRightVideo_stitching, VidInfo.xlimValCurrent);
ylim(handles.mainRightVideo_stitching, VidInfo.ylimValCurrent);
...
[tb,btns] = axtoolbar(handles.mainRightVideo_stitching, {'zoomin','zoomout','pan'});

set(handles.mainLeftVideo_stitching, 'Visible', 'off');
if isfield(handles, 'loadVideoimdisplayedStitchLeft') == 1;
    if isvalid(handles.loadVideoimdisplayedStitchLeft) == 1;
...
set(handles.timeCount_TXT_stitching, 'String', [num2str(roundn(handles.VidInfoRightStitching.TimeEC,-2)) '  /  ' num2str(roundn(handles.VidInfoRightStitching.Duration,-2))]);
set(handles.frameCount_TXT_stitching, 'String', [num2str(handles.VidInfoRightStitching.FrameEC) '  /  ' num2str(handles.VidInfoRightStitching.NbFrames)]);
...
%---Enable tools
if isempty(handles.pathLeftfile_stitching) == 0;
    %enable tools only if left video is loaded too
