function [] = pushSelectRelay_relay(varargin);
...
[file,path] = uigetfile({'*.*';'*.MP4';'*.mp4';'*.MOV';'*.mov';'*.MXF'},...
                      'Select your relay video file', handles.defaultpath);
...
disppath = fileEC;
set(handles.filenameRelay_relay, 'String', disppath);
posExtent = get(handles.filenameRelay_relay, 'Extent');
posReal = get(handles.filenameRelay_relay, 'Position');
if posExtent(3) >= posReal(3);
    disppath = [disppath(1:10) ' ... ' disppath(end-10:end)];
    set(handles.filenameRelay_relay, 'String', disppath);
end;
if ispc == 1;
    handles.pathRelayfile_relay = [path '\' fileEC];
    set(handles.filenameRelay_relay, 'tooltipstring', handles.pathRelayfile_relay);
elseif ismac == 1;
...
%     end;
% end;
% handles.pathRelayfile_relay = inputfile;
handles.pathRelayfile_relay = handles.pathRelayfile_relay;

if ismac == 1;
    ffmpegfolder = [MDIR '/SP2VideoManager/ffmpeg/bin/ffmpeg'];
    command = ['"' ffmpegfolder '" -i "' handles.pathRelayfile_relay ...
...
if status == 1;
    ...
    set(handles.filenameRelay_relay, 'tooltip', '', 'String', '');
end;
...
handles.resolutionVideoRelay = [1920 1080];
if isempty(liRes720) == 0;
    handles.resolutionVideoRelay = [1280 720];
end;
if isempty(liRes1080) == 0;
    handles.resolutionVideoRelay = [1920 1080];
end;
if isempty(liRes2160) == 0;
    handles.resolutionVideoRelay = [3840 2160];
end;

if handles.resolutionVideoRelay(1) == 1280;
    ...
    handles.CurrenCompressionRelay = 17;
    
elseif handles.resolutionVideoRelay(1) == 1920;
    ...
    handles.CurrenCompressionRelay = 35;

elseif handles.resolutionVideoRelay(1) == 3840;
    ...
    handles.CurrenCompressionRelay = 150;
end;    
set(handles.popCompression_relay, 'String', listDrop);
handles.listDropCompressionPanning = listDrop;
handles.listDropCompressionPosPanning = 1;


%---Get video
index1 = strfind(handles.pathRelayfile_relay, '''');
index2 = strfind(handles.pathRelayfile_relay, '"');
index = [index1 index2];
fileEC = handles.pathRelayfile_relay;
if isempty(index) == 0;
    fileEC(index) = [];
end;
VidInfo.name = fileEC;
...
set(handles.mainVideoRelay_relay, 'Visible', 'on');
uistack(handles.mainVideoRelay_relay, 'top');

axes(handles.mainVideoRelay_relay);
handles.loadVideoimdisplayedRelay = imshow(frame);
handles.mainVideoRelay_relay = gca;
[tb,btns] = axtoolbar(handles.mainVideoRelay_relay, {'zoomin','zoomout','pan'});
xlim(handles.mainVideoRelay_relay, VidInfo.xlimValCurrent);
ylim(handles.mainVideoRelay_relay, VidInfo.ylimValCurrent);
...
handles.sliderControl_push_relay.Data = [1:VidInfo.NbFrames];
guidata(handles.hf_w1_welcome, handles);
handles.sliderControl_push_relay.Value = 1;

set(handles.timeCount_TXT_relay, 'String', [num2str(roundn(VidInfo.TimeEC,-2)) '  /  ' num2str(roundn(VidInfo.Duration,-2))]);
set(handles.frameCount_TXT_relay, 'String', [num2str(VidInfo.FrameEC) '  /  ' num2str(VidInfo.NbFrames)]);
...
set(handles.feetOffRelayPush_relay, 'enable', 'on');
set(handles.feetOffRelayEdit_relay, 'enable', 'on', 'String', '');
set(handles.trimOutRelayPush_relay, 'enable', 'on');
set(handles.trimOutRelayEdit_relay, 'enable', 'on', 'String', '');
set(handles.RTEditRelay_relay, 'enable', 'on', 'String', '');
set(handles.popCompression_relay, 'enable', 'on', 'Value', 1);
set(handles.startProcessing_relay, 'enable', 'on');
set(handles.cancelProcessing_relay, 'enable', 'on');
set(handles.prevChapControl_push_relay, 'enable', 'on');
set(handles.prevFrameControl_push_relay, 'enable', 'on');
set(handles.nextFrameControl_push_relay, 'enable', 'on');
set(handles.nextChap_push_relay, 'enable', 'on');
set(handles.RTEditRelay_relay, 'enable', 'on');
