function [] = select_convBatchPanning_conv(varargin);

handles = guidata(gcf);
set(handles.hf_w1_welcome, 'WindowButtonDownFcn', '');

if get(handles.selectPanning_toggle_conv, 'Value') == 1;
    ...
    set(handles.batchprocessing4KPanel_conv, 'Visible', 'off');
    set(handles.batchprocessingPanningPanel_conv, 'Visible', 'on');
    handles.current_panel = 'conv';
    set(handles.select4K_toggle_conv, 'Value', 0);

else;
    %button is being pressed off
    set(handles.selectPanning_toggle_conv, 'Value', 1);
    return;    
end;
