function [] = clickCalibStitching(varargin)
...
if pt_act(1,1) >= limL & pt_act(1,1) <= limR & pt_act(1,2) >= limD & pt_act(1,2) <= limT;
    ...
    if ptEC ~= 0;
        ...
        if handles.activeVideo_stitching == 1;
            handles.ptStitchingLeft(ptEC, 1:2) = [coorU coorV];
        else;
            handles.ptStitchingRight(ptEC, 1:2) = [coorU coorV];
        end;
        ...
        if strcmpi(lower(listStitching{1,1}), 'auto') == 1;
            listStitching{1,1} = 'Manual';
            set(handles.selectPtStitching_stitching, 'String', listStitching, 'Value', ptEC+1);
        end;
    end;
end;
