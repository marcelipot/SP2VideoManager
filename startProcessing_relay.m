function [] = startProcessing_relay(varargin);
...
feefoffpanningVal = handles.feetoffVal_relay;
fileprocess = handles.pathRelayfile_relay;
RT = handles.RTVal_relay;
trimOutVal_relay = handles.trimOutVal_relay;
...
trimVal = floor(feefoffpanningVal - (RT.*VidInfo.Rate));
if trimVal <= 0;
...
set(handles.txtProgress_relay, 'String', 'Preparing video ...', 'Visible', 'on');
drawnow;
%trim frames
if ismac == 1;
    partIn = fileprocess;
...
if isempty(liRes720) == 0;
    ...
    bitrate = [num2str(handles.CurrenCompressionRelay) 'M'];
end;
...
bitrate = [num2str(handles.CurrenCompressionRelay) 'M'];
maxrate = bitrate;
bufrate = [num2str(floor((handles.CurrenCompressionRelay)./3)) 'M'];
...
if status == 0;
    ...
    set(handles.txtProgress_relay, 'String', ['Exporting video ...  ' fileoutput]);
    drawnow;
        ...
        if VidTimeOriginal - VidTimeProcessed > (trimVal*framerate);
            ...
            if missingFrames <= 1;
                ...
                if ismac == 1;
                    ...
                    if status == 0;
                        if isempty(trimOutVal_relay) == 1;
                            command = ['"' ffmpegfolder '" -accurate_seek -ss 0' ...
                                    ' -i "' outTemp4 '" -c:v copy "' ...
                                    partOut '"'];
                        else;
                            trimOutTime = ((handles.trimOutVal_relay./VidInfo.Rate) - (1/VidInfo.Rate)) - timecut;
                            command = ['"' ffmpegfolder '" -accurate_seek -ss 0' ...
                                    ' -to ' num2str(trimOutTime) ' -i "' outTemp4 '" -c:v copy "' ...
                                    partOut '"'];
                        end;
                        [status, cmdout] = system(command);
                    end;
...
set(handles.txtProgress_relay, 'String', '', 'Visible', 'off');
drawnow;
