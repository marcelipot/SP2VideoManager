function [] = pushSelectPanning_single(varargin);
...
disppath = fileEC;
set(handles.filenamePanning_single, 'String', disppath);
posExtent = get(handles.filenamePanning_single, 'Extent');
posReal = get(handles.filenamePanning_single, 'Position');
if posExtent(3) >= posReal(3);
    disppath = [disppath(1:10) ' ... ' disppath(end-10:end)];
    set(handles.filenamePanning_single, 'String', disppath);
end;
if ispc == 1;
    handles.pathPanningfile_single = [path '\' fileEC];
    set(handles.filenamePanning_single, 'tooltipstring', handles.pathPanningfile_single);
elseif ismac == 1;
...
%     end;
% end;
% handles.pathPanningfile_single = inputfile;
        
%trim frames

if ismac == 1;
    ffmpegfolder = [MDIR '/SP2VideoManager/ffmpeg/bin/ffmpeg'];
    command = ['"' ffmpegfolder '" -i "' handles.pathPanningfile_single ...
...
if status == 1;
    ...
    set(handles.filenamePanning_single, 'tooltip', '', 'String', '');
end;
...
%---Get video
index1 = strfind(handles.pathPanningfile_single, '''');
index2 = strfind(handles.pathPanningfile_single, '"');
index = [index1 index2];
fileEC = handles.pathPanningfile_single;
if isempty(index) == 0;
    fileEC(index) = [];
end;
VidInfo.name = fileEC;
...
xlim(handles.mainVideoPanning_single, VidInfo.xlimValCurrent);
ylim(handles.mainVideoPanning_single, VidInfo.ylimValCurrent);
...
[tb,btns] = axtoolbar(handles.mainVideoPanning_single, {'zoomin','zoomout','pan'});

set(handles.mainVideo4K_single, 'Visible', 'off');
if isfield(handles, 'loadVideoimdisplayed4K') == 1;
    if isvalid(handles.loadVideoimdisplayed4K) == 1;
...
set(handles.registerfeetoffPanningEdit_single, 'enable', 'on', 'String', '');
set(handles.registerfeetoff4KPush_single, 'enable', 'on');
set(handles.registerfeetoff4KEdit_single, 'enable', 'on', 'String', '');
set(handles.popCompression_single, 'enable', 'on', 'Value', 1);
